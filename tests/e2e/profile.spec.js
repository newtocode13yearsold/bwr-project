...
test.describe('Suggestions IA (profile.html)', () => {
  ...
  const MOCK_SUGGESTION = {
    ...
    mode: 'loop',
    weather: 'Ciel dégagé',
    reason: 'Météo idéale pour une sortie matinale.',
    tip: 'Partez tôt pour profiter de la fraîcheur.',
    suitability: 'ideal',
  };

  test('suggestionBlock est visible pour Silver', async ({ page }) => {
    await page.route('**/api/ai-suggestion', route => route.fulfill({
      status: 200,
      contentType: 'application/json',
    ...
    await mockAuthMe(page, 'silver');
    await injectSession(page, 'silver');
    // Clear cached suggestion to force a fresh fetch
    await page.goto('/profile.html');
    await page.evaluate(() => {
    ...
    await expect(page.locator('#suggestionBlock')).toBeVisible({ timeout: 10_000 });
  });

  test('affiche la suggestion IA reçue de l\'API', async ({ page }) => {
    await page.route('**/api/ai-suggestion', route => route.fulfill({
      status: 200,
      contentType: 'application/json',
      body: JSON.stringify(MOCK_SUGGESTION),
    ...
    await expect(page.locator('#suggestionText')).not.toContainText('L\'IA analyse', { timeout: 10_000 });
    await expect(page.locator('#suggestionText')).not.toBeEmpty();
  });

  test('utilise la suggestion mise en cache sans rappeler l\'API', async ({ page }) => {
    let apiCallCount = 0;
    await page.route('**/api/ai-suggestion', route => {
      apiCallCount++;
    ...
    // Inject a cached suggestion for today
    const weekKey = new Date().toISOString().slice(0, 7)
      + '-W' + Math.ceil(new Date().getDate() / 7);
    await page.goto('/profile.html');
    await page.evaluate(({ weekKey }) => {
      localStorage.setItem('bwr_sugg_week', weekKey);
      localStorage.setItem('bwr_sugg_html', '<p>Suggestion mise en cache</p>');
    }, { weekKey });
    apiCallCount = 0;
    await page.goto('/profile.html');

    await expect(page.locator('#suggestionText')).toContainText('Suggestion mise en cache', { timeout: 8_000 });
    expect(apiCallCount).toBe(0);
  });

  test('affiche un fallback si l\'API échoue', async ({ page }) => {
    ...
    await expect(page.locator('#suggestionBlock')).toBeVisible({ timeout: 10_000 });
    // After fallback the text should not be stuck on the loading state
    await expect(page.locator('#suggestionText')).not.toContainText('L\'IA analyse', { timeout: 8_000 });
    await expect(page.locator('#suggestionText')).not.toBeEmpty();
  });

  test('suggestionBlock est visible pour Gold avec cadence quotidienne', async ({ page }) => {
    await page.route('**/api/ai-suggestion', route => route.fulfill({
      status: 200,
      contentType: 'application/json',
      body: JSON.stringify(MOCK_SUGGESTION),
  ...
  test('le bouton Générer ce trajet pointe vers routes.html', async ({ page }) => {
    await page.route('**/api/ai-suggestion', route => route.fulfill({
      status: 200,
      contentType: 'application/json',
      body: JSON.stringify(MOCK_SUGGESTION),
    ...
    const btn = page.locator('#suggestionBtn');
    await expect(btn).toBeVisible({ timeout: 10_000 });
    const href = await btn.getAttribute('href');
    expect(href).toMatch(/routes\.html/);
  });
});
