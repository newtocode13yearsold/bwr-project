#!/usr/bin/env node
// ─────────────────────────────────────────────────────────────────────────────
//  BWR Instagram reel — "Boucles" (la carte vivante, marimba bed, no voice)
//
//  Records the self-contained reel page `public/ads/promo-loop.html`, which
...
//    PROMO_BASE_URL=http://localhost:4810 node scripts/promo/make-loop.mjs
//    node scripts/promo/make-loop.mjs --encode-only   # re-use last recording
//    node scripts/promo/make-loop.mjs --only=9x16       # one format only
// ─────────────────────────────────────────────────────────────────────────────
...
const SUF = FEED ? '-feed' : '';
const RAW_DIR = join(__dirname, '.raw-loop' + SUF);
const MASTER = join(OUT_DIR, 'bwr-loop' + SUF + '-master.webm');
const LOOP   = join(OUT_DIR, 'bwr-loop-loop.wav'); // exact synth bed from the page
const BED    = join(OUT_DIR, 'bwr-loop-bed.m4a');  // trimmed/faded/normalized bed
...
const PAGE = '/ads/promo-loop.html?bare=1';

// The page's own timeline length (kept in sync with TOTAL + CTA tail in promo-loop.html).
const REEL_MS = 20500;
...
async function encode() {
  ...
  for (const f of targets) {
    const out = join(OUT_DIR, `bwr-loop${SUF}-${f.id}.mp4`);
    const common = [
    ...
    await run(a);
    const drop = join(DROP, `BWR-pub-loop${SUF}-${f.id}.mp4`);
...
(async () => {
  try {
    ...
    await encode();
    console.log('\n✅ Boucles reel ready in ../add/ — BWR-pub-loop-9x16.mp4 drops straight into Instagram.');
  } catch (e) {
    console.error('\n❌ loop reel build failed:', e.message);
    process.exit(1);
  }
})();
