...
//
//  Records the self-contained reel page `public/ads/promo-solve.html` FIVE times,
...
//       the shared ad drop folder (…/Projet Thomas/add) as BWR-pub-solve-N-*.mp4.
//
...
const LOOP    = join(OUT_DIR, 'bwr-solve-loop.wav');            // exact synth bed
const BED     = join(OUT_DIR, 'bwr-solve-bed.m4a');             // trimmed/faded bed

const BASE_URL = process.env.PROMO_BASE_URL || 'http://localhost:4810';
const PAGE = '/ads/promo-solve.html?bare=1';

const REEL_MS = 16500;      // page timeline + CTA tail (TOTAL=16000 in the HTML)
const AUDIO_SECONDS = 20;   // render a bit longer; ffmpeg trims to fit
const FINAL_SEC = 16.5;
...
const master = v => join(OUT_DIR, `bwr-solve-${v}-master.webm`);
...
async function recordVariant(browser, ctxOpts, v) {
  const RAW = join(__dirname, `.raw-solve-${v}`);
...
async function encodeVariant(v) {
  ...
  for (const f of targets) {
    const out = join(OUT_DIR, `bwr-solve-${v}-${f.id}.mp4`);
    const common = [
    ...
    process.stdout.write(`▶ encoding solve-${v} ${f.id} (${f.w}×${f.h}) … `);
    await run(a);
    await mkdir(DROP, { recursive: true });
    const drop = join(DROP, `BWR-pub-solve-${v}-${f.id}.mp4`);
...
(async () => {
  try {
    ...
    console.log('\n✅ Série « problème → solution » prête dans ../add/ — BWR-pub-solve-1..5-9x16.mp4');
  } catch (e) {
