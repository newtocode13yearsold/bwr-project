#!/usr/bin/env node
// ─────────────────────────────────────────────────────────────────────────────
//  BWR Instagram reel — "Difficulté" (la carte vivante, marimba bed, no voice)
//
//  Records the self-contained reel page `public/ads/promo-grades.html`, which
...
//    PROMO_BASE_URL=http://localhost:4810 node scripts/promo/make-grades.mjs
//    node scripts/promo/make-grades.mjs --encode-only   # re-use last recording
//    node scripts/promo/make-grades.mjs --only=9x16       # one format only
// ─────────────────────────────────────────────────────────────────────────────
...
const SUF = FEED ? '-feed' : '';
const RAW_DIR = join(__dirname, '.raw-grades' + SUF);
const MASTER = join(OUT_DIR, 'bwr-grades' + SUF + '-master.webm');
const LOOP   = join(OUT_DIR, 'bwr-grades-loop.wav'); // exact synth bed from the page
const BED    = join(OUT_DIR, 'bwr-grades-bed.m4a');  // trimmed/faded/normalized bed
...
const PAGE = '/ads/promo-grades.html?bare=1';

// The page's own timeline length (kept in sync with TOTAL + CTA tail in promo-grades.html).
const REEL_MS = 20500;
...
async function encode() {
  ...
  for (const f of targets) {
    const out = join(OUT_DIR, `bwr-grades${SUF}-${f.id}.mp4`);
    const common = [
    ...
    await run(a);
    const drop = join(DROP, `BWR-pub-grades${SUF}-${f.id}.mp4`);
...
(async () => {
  try {
    ...
    await encode();
    console.log('\n✅ Difficulté reel ready in ../add/ — BWR-pub-grades-9x16.mp4 drops straight into Instagram.');
  } catch (e) {
    console.error('\n❌ grades reel build failed:', e.message);
    process.exit(1);
  }
})();
