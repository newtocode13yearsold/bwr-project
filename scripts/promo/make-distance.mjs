#!/usr/bin/env node
// ─────────────────────────────────────────────────────────────────────────────
//  BWR Instagram reel — "Le temps que tu as" (retour pile à la voiture)
//
//  Records the self-contained reel page `public/ads/promo-distance.html`, which
//  plays its own ~18.5 s motion-graphics timeline (phone mockup on the real IGN
//  map: pick your time → a real graph-router loop appears and comes back to the
//  car; change the time → longer loop, same car). Same steps as
//  make-report-real.mjs / make-loop.mjs:
...
//    PROMO_BASE_URL=http://localhost:8787 node scripts/promo/make-distance.mjs
//    node scripts/promo/make-distance.mjs --encode-only   # re-use last recording
// ─────────────────────────────────────────────────────────────────────────────
...
const SUF = FEED ? '-feed' : '';
const RAW_DIR = join(__dirname, '.raw-distance' + SUF);
const MASTER = join(OUT_DIR, 'bwr-distance' + SUF + '-master.webm');
const LOOP   = join(OUT_DIR, 'bwr-distance-loop.wav'); // exact synth bed from the page
const BED    = join(OUT_DIR, 'bwr-distance-bed.m4a');  // trimmed/faded/normalized bed
...
const PAGE = '/ads/promo-distance.html?bare=1';

// The page's own timeline length (kept in sync with TOTAL + CTA tail in the HTML).
const REEL_MS = 18550;
const AUDIO_SECONDS = 23; // render a little longer than the reel; ffmpeg trims to fit
const FINAL_SEC = 18.55;
...
async function encode() {
  ...
  for (const f of targets) {
    const out = join(OUT_DIR, `bwr-distance${SUF}-${f.id}.mp4`);
    const common = [
    ...
    await run(a);
    const drop = join(DROP, `BWR-pub-distance${SUF}-${f.id}.mp4`);
...
(async () => {
  try {
    ...
    await encode();
    console.log('\n✅ "Le temps que tu as" reel ready in ../add/ — BWR-pub-distance-9x16.mp4 drops straight into Instagram.');
  } catch (e) {
    console.error('\n❌ distance reel build failed:', e.message);
    process.exit(1);
  }
})();
