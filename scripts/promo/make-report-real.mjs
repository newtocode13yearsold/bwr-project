...
//
//  Records the self-contained reel page `public/ads/promo-report-real.html`, which
...
//    PROMO_BASE_URL=http://localhost:4810 node scripts/promo/make-report-real.mjs
//    node scripts/promo/make-report-real.mjs --encode-only   # re-use last recording
//    node scripts/promo/make-report-real.mjs --only=9x16       # one format only
// ─────────────────────────────────────────────────────────────────────────────
...
const SUF = FEED ? '-feed' : '';
const RAW_DIR = join(__dirname, '.raw-report-real' + SUF);
const MASTER = join(OUT_DIR, 'bwr-report-real' + SUF + '-master.webm');
const LOOP   = join(OUT_DIR, 'bwr-report-real-loop.wav'); // exact synth bed from the page
const BED    = join(OUT_DIR, 'bwr-report-real-bed.m4a');  // trimmed/faded/normalized bed
...
const PAGE = '/ads/promo-report-real.html?bare=1';

// The page's own timeline length (kept in sync with TOTAL + CTA tail in promo-report-real.html).
const REEL_MS = 20500;
...
async function encode() {
  ...
  for (const f of targets) {
    const out = join(OUT_DIR, `bwr-report-real${SUF}-${f.id}.mp4`);
    const common = [
    ...
    await run(a);
    const drop = join(DROP, `BWR-pub-report-real${SUF}-${f.id}.mp4`);
...
(async () => {
  try {
    ...
    await encode();
    console.log('\n✅ Signalements reel ready in ../add/ — BWR-pub-report-real-9x16.mp4 drops straight into Instagram.');
  } catch (e) {
