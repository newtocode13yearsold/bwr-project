#!/usr/bin/env node
// ─────────────────────────────────────────────────────────────────────────────
//  BWR Instagram reel — "TOUR" : full-product presentation in one reel.
//
//  Records the self-contained reel page `public/ads/promo-tour.html` ONCE.
//  The reel opens on a real forest photo, then BWR's REAL map slides in and the
//  reel walks through every headline feature (A→B, boucles, difficulté notée,
//  signalements live, hors-ligne, journal, badges/défis) before the CTA.
//  Same synth marimba bed + encode plumbing as the other make-*.mjs scripts.
//
//  Output: the shared "add 2" drop folder next to the repo, named `add 1.mp4`
//  (…/Projet Thomas/add 2/add 1.mp4).
//
...
//    PROMO_BASE_URL=http://localhost:4810 node scripts/promo/make-tour.mjs
//    node scripts/promo/make-tour.mjs --only=9x16      # only the vertical cut
//    node scripts/promo/make-tour.mjs --encode-only    # re-use the recording
// ─────────────────────────────────────────────────────────────────────────────
...
const DROP_NAME = 'add 1';                                     // requested file name
const LOOP    = join(OUT_DIR, 'bwr-tour-loop.wav');
const BED     = join(OUT_DIR, 'bwr-tour-bed.m4a');

const BASE_URL = process.env.PROMO_BASE_URL || 'http://localhost:4810';
const PAGE = '/ads/promo-tour.html?bare=1';
...
const master = join(OUT_DIR, 'bwr-tour-master.webm');
...
async function recordReel(browser, ctxOpts) {
  const RAW = join(__dirname, '.raw-tour');
...
async function encode() {
  ...
  for (const f of targets) {
    const out = join(OUT_DIR, `bwr-tour-${f.id}.mp4`);
    const common = [
    ...
    process.stdout.write(`▶ encoding tour ${f.id} (${f.w}×${f.h}) … `);
    await run(a);
