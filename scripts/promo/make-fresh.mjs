...
//
//  Records the self-contained reel page `public/ads/promo-fresh.html` FIVE times,
...
//       the shared ad drop folder (…/Projet Thomas/add) as BWR-pub-fresh-N-*.mp4.
//
...
const LOOP    = join(OUT_DIR, 'bwr-fresh-loop.wav');            // exact synth bed
const BED     = join(OUT_DIR, 'bwr-fresh-bed.m4a');             // trimmed/faded bed

const BASE_URL = process.env.PROMO_BASE_URL || 'http://localhost:4810';
const PAGE = '/ads/promo-fresh.html?bare=1';
...
const master = v => join(OUT_DIR, `bwr-fresh-${v}-master.webm`);
...
async function recordVariant(browser, ctxOpts, v) {
  const RAW = join(__dirname, `.raw-fresh-${v}`);
...
async function encodeVariant(v) {
  ...
  for (const f of targets) {
    const out = join(OUT_DIR, `bwr-fresh-${v}-${f.id}.mp4`);
    const common = [
    ...
    process.stdout.write(`▶ encoding fresh-${v} ${f.id} (${f.w}×${f.h}) … `);
    await run(a);
    await mkdir(DROP, { recursive: true });
    const drop = join(DROP, `BWR-pub-fresh-${v}-${f.id}.mp4`);
...
(async () => {
  try {
    ...
    console.log('\n✅ Série « problème → solution » prête dans ../add/ — BWR-pub-fresh-1..5-9x16.mp4');
  } catch (e) {
