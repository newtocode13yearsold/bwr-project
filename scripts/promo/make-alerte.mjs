#!/usr/bin/env node
// ─────────────────────────────────────────────────────────────────────────────
//  BWR Instagram reel — "Alertes obstacle" (obstacle push alerts on saved routes)
//
//  Records the self-contained reel page `public/ads/promo-alerte.html`, which
//  plays its own ~20 s motion-graphics timeline (phone mockup: a saved route on
//  the REAL IGN map → a fallen-tree pin drops on it → BWR pushes a notification
//  banner → payoff). Same pipeline as make-grades.mjs / the other make-*.mjs:
...
//    PROMO_BASE_URL=http://localhost:4810 node scripts/promo/make-alerte.mjs
//    node scripts/promo/make-alerte.mjs --encode-only   # re-use last recording
//    node scripts/promo/make-alerte.mjs --only=9x16       # one format only
// ─────────────────────────────────────────────────────────────────────────────
...
const SUF = FEED ? '-feed' : '';
const RAW_DIR = join(__dirname, '.raw-alerte' + SUF);
const MASTER = join(OUT_DIR, 'bwr-alerte' + SUF + '-master.webm');
const LOOP   = join(OUT_DIR, 'bwr-alerte-loop.wav'); // exact synth bed from the page
const BED    = join(OUT_DIR, 'bwr-alerte-bed.m4a');  // trimmed/faded/normalized bed
const DROP   = join(__dirname, '..', '..', '..', 'add');

const BASE_URL = process.env.PROMO_BASE_URL || 'http://localhost:4810';
const PAGE = '/ads/promo-alerte.html?bare=1';

// The page's own timeline length (kept in sync with TOTAL + CTA tail in promo-alerte.html).
const REEL_MS = 20500;
...
async function encode() {
  ...
  for (const f of targets) {
    const out = join(OUT_DIR, `bwr-alerte${SUF}-${f.id}.mp4`);
    const common = [
    ...
    await run(a);
    const drop = join(DROP, `BWR-pub-alerte${SUF}-${f.id}.mp4`);
...
(async () => {
  try {
    ...
    await encode();
    console.log('\n✅ Alertes-obstacle reel ready in ../add/ — BWR-pub-alerte-9x16.mp4 drops straight into Instagram.');
  } catch (e) {
    console.error('\n❌ alerte reel build failed:', e.message);
    process.exit(1);
  }
})();
