...
async function loadErrors() {
  ...
  try {
    ...
    // Breakdown strip: which pages and devices are worst hit (weighted by count).
    const bd = document.getElementById('errBreakdown');
    if (bd) {
      if (!errors.length) { bd.style.display = 'none'; }
      else {
        const tally = (key) => {
          const m = {};
...
async function loadContentStats() {
  ...
  try {
    ...
    const totalKm = pathArr.reduce((s, p) => s + (Number(p.distanceKm) || 0), 0);
...
async function loadMembers() {
  ...
  try {
    ...
    list.innerHTML = users.map(u => {
      ...
      // Contribution chips — only render the ones with a non-zero value to keep rows tidy.
      const chips = [];
      ...
      const statsLine = chips.length
        ? `<div style="margin-top:4px;display:flex;gap:6px;flex-wrap:wrap">${chips.map(c =>
