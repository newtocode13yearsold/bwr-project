...
- Name: map.spec.js >> Carte (map.html) >> le panneau de filtres est accessible
- Location: tests\e2e\map.spec.js:57:7
...
Locator: locator('#surfaceFilter, [data-filter], .filter-btn').first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('#surfaceFilter, [data-filter], .filter-btn').first()
