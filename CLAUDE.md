...
- Auth: /api/setup (one-time admin creation), /api/auth/{register,login,logout,me,profile,password,account} — NB: PUT /api/auth/profile applies a name change immediately, but an **email** change is a two-step confirmation: it re-checks the current password, emails a link to the NEW address, and only swaps the login email when GET /api/auth/verify-email-change?token=… is opened (see `emailchange:` KV key). Frontend confirmation page: `public/verify-email.html` + `js/verify-email.js`., /api/auth/plan/:userId (admin plan changes), /api/auth/stats, /api/auth/consume-route (free weekly quota), /api/auth/wheel-prize, /api/auth/start-trial (self-service one-time free 7-day Silver trial — free accounts only, gated by `silverTrialUsed`; reuses the `planExpiresAt`/`planBase` revert-on-expiry path in `/api/auth/me`), POST /api/auth/onboarded (auth — one-way switch that marks the first-run tour seen: sets `user.onboarded = true`, never clears it). **First-run onboarding:** a brand-new account is created at email-verify with `onboarded: false`; legacy accounts lack the field and map to `true`. `login` + `/api/auth/me` both surface `onboarded` (`updated.onboarded !== false`). The one-time post-signup welcome + coach-mark tour (`public/js/onboarding.js`, gated on the cached user's `onboarded === false`) shows **exactly once per account, right after signup, and can never be replayed** — it POSTs `/api/auth/onboarded` the instant it appears. There is deliberately no replay entry point (no `?tour=` param, no reset).
...
- Analytics: POST /api/track/visit (public — per-page dwell tracking. `public/js/track.js` measures the *visible* time on each page (to the second, paused while the tab is hidden) and sends `{ vid, page, seconds }` on `pagehide` (plus the auth `token` when signed in). **Admin's own browsing is never counted:** `track.js` sets a persistent per-browser `bwr_notrack` opt-out once it sees a cached admin user, and — because that flag misses origins where no admin login was ever cached (preview deploys, incognito, cleared storage) — the server independently drops the visit when the beacon's `token` maps to an admin session (`role === 'admin'`). The server accumulates this into one record per anonymous visitor per month; a visitor is only *counted* (and shown in the admin list) once their cumulative time crosses the `THRESHOLD` (10 s) so bots/bounces are excluded — records under the bar are stored with `counted:false` and filtered out of the list. A server-side User-Agent bot filter (`isBotUA` in `worker/handlers/admin.js`) is a second line of defence — a missing UA is treated as a bot. Each record stores an anonymous `vid`, coarse Cloudflare city/country/region from `request.cf`, a friendly device label from `describeDevice` (never the IP or raw UA), `firstSeen`/`lastSeen`, `visits` (page views), total `seconds`, `counted`, and a `pages` map of `{ path: { seconds, views } }` rendered per-visitor in the admin "Activité" tab), GET /api/analytics/events (admin — recent login/signup events + `totalLogins`/`totalSignups` + `monthlyVisits`/`visitsThisMonth` + `visitors` (per-person list for the current month, most-recently-active first, capped at 500 with `visitorsTruncated`)). Frontend renders the per-visitor list in the profile-less admin "Activité" tab (`loadVisits` in `public/js/admin.js`).
- Error monitoring (`worker/handlers/errors.js`, homegrown Sentry-lite): every uncaught JS error and unhandled promise rejection on a real user's device is beaconed to the server, grouped, and pushes the admin a ntfy alert. **`public/js/errors.js`** (loaded on every real page, right after `config.js` so it catches later scripts) registers `window.onerror` + `unhandledrejection`, dedups per page-load (caps ~12 sends), skips browser-extension/opaque cross-origin noise, and `sendBeacon`s `{kind, message, stack, source, line, col, page}` to **POST /api/track/error** (public, no auth — errors happen before/without login; text/plain to stay CORS-simple). The server drops bot UAs (`isBotUA`, missing UA = bot), groups identical errors under an FNV-1a **signature** of `kind|message|topStackFrame|page` (so a bug that fires 1 000× is one row with a `count`), stores `errlog:{sig}` (30-day TTL), and fires a **throttled** ntfy push (`bwr-ciril8596`, Tag `bug`) only on a new signature or a recurrence after a 6 h quiet spell. On a `shouldAlert` it ALSO emails `env.ADMIN_EMAIL` via the shared `sendEmail` (Resend) helper — same throttle, no-ops in dev / when unset. Admin endpoints: GET /api/errors (`{errors[], distinct, total}`, newest-`lastSeen` first), GET /api/errors/count (`{count}` distinct unresolved — cheap `listKeys`-only poll for the nav badge), DELETE /api/errors/:sig (resolve one), DELETE /api/errors (clear all). Frontend: the "🐞 Erreurs JS" card in `admin-panel.html` (`loadErrors` in `public/js/admin.js`), plus **`public/js/admin-badge.js`** (loaded site-wide right after `errors.js`) which, for a cached admin, polls the count and paints a red badge on the "Panneau admin" nav entry — it creates the badge element itself (no per-page nav markup) and exposes `window.__bwrRefreshErrBadge()` so `loadErrors` re-syncs it after a resolve. **NB: `errors.js` reads `API_URL` even when it's `''` (same-origin) — falling back to the canonical host only when it is genuinely `undefined`, so a dev/preview page never beacons its errors to production.** Tests: `tests/worker-errors.test.mjs`.
...
- Recorded activities / hike journal (`worker/handlers/activities.js`, **all signed-in users** — this is the retention hook, deliberately not plan-gated): the Strava-style "record a walk → save it → replay it later" loop. POST /api/activities ({name, coords:[[lat,lon]], elevations?, times?, meters, seconds, movingSeconds?, ascent?, descent?, startedAt} — server sanitises the track: finite/in-range coords only, parallel `elevations`/`times` kept only when their length matches the track, stats clamped ≥ 0, ≤ `MAX_POINTS` 50 000 points), GET /api/activities (list summaries — coords/elevations/times stripped, a light `points` count added, newest-first), GET /api/activities/:id (full track for replay + GPX), PATCH /api/activities/:id ({name} rename), DELETE /api/activities/:id. All scoped by the `activity:{userId}:` key prefix, so one user can't read another's. **Recording lives in `public/js/gps-tracker.js`** (map/admin pages): it now records the track (points + timestamps + GPS altitude) alongside the km counter and, on stop, offers a save-to-journal modal (signed-in only); ascent/descent are computed client-side from GPS altitude with a 3 m noise threshold. **Distance is measured through the shared Kalman filter `public/js/gps-filter.js` (`createGpsDistanceFilter`), NOT by naively summing raw fixes** — both live trackers (`gps-tracker.js` and the `GpsTracker` in `public/js/routes.js`) feed every `watchPosition` fix through it. It runs an accuracy-gated constant-velocity Kalman filter (measurement variance = the fix's reported `accuracy²`) and only adds distance once the smoothed move clears a dead-band tied to the filter's own uncertainty, so GPS jitter no longer inflates the total (the old flat 40 m accuracy gate + 5 m floor over-counted a 1 km walk to 3–5 km and invented ~1 km while standing still). Marker + saved track use the *smoothed* position. `gps-filter.js` must load before both consumers (added to `map.html`, `admin.html`, `routes.html` and the SW `APP_SHELL`); it also `module.exports` for tests. Tune via the `DEFAULTS` in the module (`maxAccuracyM`, `q`, `maxSpeedKmh`, `minMoveM`, `deadbandK`). Tests: `tests/gps-filter.test.js`. **Journal UI: `public/activities.html` + `public/js/activities.js`** — list with per-activity distance/duration/pace/ascent, rename, delete, GPX export (reuses `downloadGPX` in `js/exporters.js`), and an inline Leaflet replay modal (animated marker + scrubber, ~12 s regardless of walk length). Activities are included in the GDPR export and purged on account deletion (`worker/handlers/auth.js`). Tests: `tests/worker-activities.test.mjs`.
...
- follow:{followerId}:{followeeId} — ISO timestamp (existence = follower follows followee; prefix `follow:{me}:` = who I follow)
- follower:{followeeId}:{followerId} — ISO timestamp (reverse index; prefix `follower:{me}:` = who follows me). Both keys are written/deleted together; account deletion purges both directions.
...
- errlog:{sig} — JSON client-error group {sig, kind, message, stack, source, line, col, page, device, count, firstSeen, lastSeen, lastAlertAt}; sig = FNV-1a of `kind|message|topStackFrame|page`; 30-day TTL (old errors self-expire)
...
- friends.html + js/friends.js: Communauté — the social layer. Three tabs (Fil d'actu = shared-activity feed, Découvrir = user directory to find & follow randonneurs, Mon réseau = following/followers). Kudos + inline Leaflet replay of any shared walk. All signed-in users.
...
through community contributions, not distance** — `XP = reports + pathGrades×2`
...
| `tests/gps-filter.test.js` | GPS distance Kalman filter — accuracy gate, stationary jitter rejection, real-walk accuracy vs naive sum, teleport-spike rejection, reset | CJS, Node test runner |
