...
- analytics:visits:{YYYY-MM} — integer count of unique anonymous visitors that month (dwell-gated > 30 s), ~13-month TTL
- visitor:{YYYY-MM}:{vid} — JSON per-visitor record {vid, firstSeen, lastSeen, visits, country, city, region, device}; existence also serves as the once-per-month dedup marker (replaces the old `vseen:` marker), ~13-month TTL
